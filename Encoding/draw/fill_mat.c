...
void fill_mat_ep(char **mat, size_t size, int version, char *msg, size_t msg_length)
{
    size_t version_m = 0;
    int nv = 0;
    if(version >= 7)
    ...
    while(1)
    {
        ...
        {   
            //warn("up 1");
            //warn("%d %d", i, j);
            downwards_ep(mat, msg, size, &i, &j, &k);
            //warn("%d %d", i, j);
        }
        else if (stop == 2) // in a c (maybe c then t)
        {
...
static inline
void upwards_ep(char **mat, char *msg, size_t size, int *ip, int *jp, int *kp)
{
    int k = *kp;
    int i = *ip;
    int j = *jp;
    while(((size_t)j < size && (size_t)i < size) && 
            (i >= 0 && j  >= 0) && (mat[i][j] == 0))
    {
        /* In our Epicode we wish to stack 2 bits into one point, to achieve this
         * we will use color, such that :
         *  initial msg = 001100100011110001100000
         *  ##### [i,j] bit in the matrix will be :
         *  00                                      (bits 0 to 1 in the msg)
         *  decomposed we have : 0 - 0 
         *  which correspond to: white
         *  so mat[i,j] will be 3 and represented by a blue color
         */
...
static inline
void downwards_ep(char **mat, char *msg, size_t size, int *ip, int *jp, int *kp)
{
    int k = *kp;
    int i = *ip;
    int j = *jp;
